...
public class InsertTestData {
    public static void main(String[] args) {
        try {
            ...
            // 1. 🌟 Απενεργοποιούμε προσωρινά τους ελέγχους Foreign Keys για τον καθαρισμό
            conn.createStatement().execute("PRAGMA foreign_keys = OFF");

            // 2. 🧹 Καθαρίζουμε όλους τους πίνακες με απόλυτη ασφάλεια
            conn.createStatement().execute("DELETE FROM ProfessorCourse");
            conn.createStatement().execute("DELETE FROM StudentCourse");
            conn.createStatement().execute("DELETE FROM StudyPlan");
            conn.createStatement().execute("DELETE FROM StudentProfile");
            conn.createStatement().execute("DELETE FROM CourseEvaluation");
            conn.createStatement().execute("DELETE FROM HelpOffer");
            conn.createStatement().execute("DELETE FROM InterviewMeeting");
            conn.createStatement().execute("DELETE FROM ThesisSlot");
            conn.createStatement().execute("DELETE FROM Thesis");
            conn.createStatement().execute("DELETE FROM AvailabilitySlot");
            conn.createStatement().execute("DELETE FROM Course");
            conn.createStatement().execute("DELETE FROM Student");
            conn.createStatement().execute("DELETE FROM Professor");
            conn.createStatement().execute("DELETE FROM Secretary");
            conn.createStatement().execute("DELETE FROM Notification");
            conn.createStatement().execute("DELETE FROM User");

            // 3. 🌟 Κρατάμε OFF τους ελέγχους κατά τα inserts για να περάσουν όλα "αέρα" χωρίς constraints errors
            conn.createStatement().execute("PRAGMA foreign_keys = OFF");

            System.out.println("🧹 Η βάση δεδομένων καθαρίστηκε πλήρως και είναι έτοιμη για φρέσκια εισαγωγή!");
            ...
            conn.createStatement().execute("""
                ...
                ('st17','st17','hash','st1030017@ceid.upatras.gr','Panagiotis','Oikonomου',     datetime('now')),
                ...
                -- 🌟 ΔΙΟΡΘΩΣΗ: Προσθήκη των test χρηστών στον πίνακα User για να μην σκάει το Foreign Key Constraint
                ...
                ('2002', 'test_user', 'hash', 'test@ceid.upatras.gr', 'Test', 'Student', datetime('now'))
            """);
            System.out.println("✅ Users-Students (43) προστέθηκαν!");
            ...
            conn.createStatement().execute("""
                ...
                VALUES 
                (1, 'p1','Κτίριο Β-101',3,3),
                (2, 'p2','Κτίριο Β-102',3,3),
                (3, 'p3','Κτίριο Α-201',3,2),
                (4, 'p4','Κτίριο Α-205',3,0),
                (5, 'p5','Κτίριο Β-305',3,1),
                (6, 'p6','Κτίριο Α-110',3,0),
                (7, 'p7','Κτίριο Β-210',3,0),
                (8, 'p8','Κτίριο Α-301',3,0),
                (9, 'p9','Κτίριο Β-401',3,0),
                (10,'p10','Κτίριο Α-105',3,0),
                (11,'p11','Κτίριο Β-205',3,0),
                (12,'p12','Κτίριο Α-310',3,0),
                (13,'p13','Κτίριο Β-115',3,0),
                (14,'p14','Κτίριο Α-215',3,0),
                (15,'p15','Κτίριο Β-315',3,0),
                (16,'p16','Κτίριο Α-415',3,0),
                (17,'p17','Κτίριο Β-120',3,0),
                (18,'p18','Κτίριο Α-220',3,0),
                (19,'p19','Κτίριο Β-320',3,0),
                (20,'p20','Κτίριο Α-420',3,0),
                (21,'p21','Κτίριο Β-125',3,0),
                (22,'p22','Κτίριο Α-225',3,0),
                (23,'p23','Κτίριο Β-325',3,0),
                (24,'p24','Κτίριο Α-425',3,0),
                (25,'p25','Κτίριο Β-130',3,0)
            """);
            ...
            conn.createStatement().execute("""
                ...
                (2002, '2002', 2022, 5, 120, 1, 'GPA:7.5')
            """);
            System.out.println("✅ Students (43) προστέθηκαν!");
            ...
            conn.createStatement().execute("""
                ...
                VALUES 
                ('CEID_22Y101','Διακριτά Μαθηματικά','Τομέας ΕΘ',7,1,0,0,'',1,7.2,6.5),
                ('CEID_22Y102','Γραμμική Άλγεβρα','Τομέας ΕΘ',7,1,0,0,'',1,7.0,6.2),
                ('CEID_22Y103','Εισαγωγή στον Προγραμματισμό','Τομέας ΛΥ',9,1,0,0,'',1,8.1,7.5),
                ('CEID_22Y104','Βασικές Αρχές Οργάνωσης Υπολογιστικών Συστημάτων','Τομέας YA',7,1,0,0,'',1,7.5,6.0),
                ('CEID_22Y105','Γενικά Μαθηματικά Ι','Τομέας ΕΘ',7,2,0,0,'',1,6.8,7.0),
                ('CEID_23Y106','Αντικειμενοστρεφής Προγραμματισμός','Τομέας ΛΥ',9,2,0,0,'',1,8.5,7.8),
                ('CEID_23Y107','Λογική Σχεδίαση','Τομέας YA',7,2,0,0,'',1,7.3,6.5),
                ('CEID_22Y108','Ηλεκτρικά Κυκλώματα','Τομέας YA',7,2,0,0,'',1,6.9,6.8),
                ('CEID_23Y201','Γενικά Μαθηματικά ΙΙ','Τομέας ΕΘ',6,3,0,0,'',1,6.5,6.8),
                ('CEID_23Y202','Θεωρία Γραφημάτων και Εφαρμογές','Τομέας ΕΘ',6,3,0,0,'',1,7.8,6.5),
                ('CEID_23Y203','Αρχιτεκτονική Υπολογιστών','Τομέας YA',4,3,0,0,'',1,7.1,6.3),
                ('CEID_23Y204','Πιθανότητες και Αρχές Στατιστικής','Τομέας ΕΘ',6,3,0,0,'',1,7.1,6.8),
                ('CEID_23Y205','Εισαγωγή στους Αλγόριθμους','Τομέας ΕΘ',6,3,0,0,'',1,8.0,7.2),
                ('CEID_23Y211','Εργαστήριο Λογικού Σχεδιασμού','Τομέας YA',2,3,0,0,'',1,7.5,5.0),
                ('CEID_23Y206','Αρχές Γλωσσών Προγραμματισμού και Μεταφραστών','Τομέας ΛΥ',6,4,0,0,'',1,7.5,7.0),
                ('CEID_23Y207','Θεωρία Σημάτων και Συστημάτων','Τομέας YA',6,4,0,0,'',1,7.0,7.2),
                ('CEID_23Y208','Αναλογικά και Ψηφιακά Ηλεκτρονικά','Τομέας YA',4,4,0,0,'',1,6.8,6.5),
                ('CEID_23Y209','Αριθμητική Ανάλυση και Περιβάλλοντα Υλοποίησης','Τομέας ΛΥ/ΕΘ',6,4,0,0,'',1,7.2,6.8),
                ('CEID_23Y210','Δομές Δεδομένων','Τομέας ΛΥ',6,4,0,0,'',1,7.9,7.0),
                ('CEID_23Y212','Εργαστήριο Αρχιτεκτονικής Υπολογιστών','Τομέας YA',2,4,0,0,'',1,7.3,5.0),
                ('CEID_24Y303','Εργαστήριο Αναλογικών & Ψηφιακών Ηλεκτρονικών','Τομέας YA',2,5,0,0,'',1,7.0,5.0),
                ('CEID_24Y330','Λειτουργικά Συστήματα','Τομέας ΛΥ',6,5,0,0,'',1,7.6,7.0),
                ('CEID_24Y334','Βάσεις Δεδομένων','Τομέας AY',6,5,0,0,'',1,8.2,6.8),
                ('CEID_24Y351','Τεχνητή Νοημοσύνη','Τομέας ΛΥ',6,5,0,0,'',1,8.4,7.3),
                ('CEID_24Y361','Συστήματα Μικροϋπολογιστών','Τομέας YA',5,5,0,0,'',1,7.2,6.5),
                ('CEID_24Y381','Ψηφιακή Επεξεργασία Σημάτων','Τομέας YA',5,5,0,0,'',1,7.4,7.0),
                ('CEID_24Y387','Δίκτυα Υπολογιστών','Τομέας YA',6,6,0,0,'',1,7.8,7.0),
                ('CEID_24Y302','Βασικές Έννοιες Συστημάτων Επικοινωνίας','Τομέας YA',6,6,0,0,'',1,7.1,6.8),
                ('CEID_24Y332','Τεχνολογία Λογισμικού','Τομέας ΛΥ',6,6,0,0,'',1,7.5,6.5),
                ('CEID_24Y338','Προγραμματισμός και Συστήματα στον Παγκόσμιο Ιστό','Τομέας ΛΥ',6,6,0,0,'',1,8.7,7.8),
                ('CEID_24Y301','Θεωρία Υπολογισμού και Πολυπλοκότητα','Τομέας ΕΘ',6,6,0,0,'',1,7.4,7.5),
                ('CEID_25Y401','Προγραμματισμός Συστημάτων','Υποχρεωτικό 7ου - ΥΑ/ΛΥ',5,7,0,0,'',1,7.8,7.2),
                
                -- === ΜΑΘΗΜΑΤΑ ΥΠΟΧΡΕΩΤΙΚΑ ΚΑΤ' ΕΠΙΛΟΓΗΝ ===
                ('CEID_NE5057','Αλγόριθμοι και Συνδυαστική Βελτιστοποίηση','Τομέας ΕΘ',5,7,1,0,'K1:A',1,7.9,8.0),
                ('CEID_NE4117','Κατανεμημένα Συστήματα','Τομέας ΕΘ',5,7,1,1,'K1:A,K2:B,K4:B,K6:B',1,8.1,7.5),
                ('CEID_NE5015','Όρια Υπολογισμού & Αλγοριθμικές Μέθοδοι Επίλυσης','Τομέας ΕΘ',5,8,1,1,'K1:A,K2:B',1,7.3,7.6),
                ('CEID_NE5017','Πιθανοτικές Τεχνικές και Τυχαιοκρατικοί Αλγόριθμοι','Τομέας ΕΘ',5,7,1,0,'K1:A',1,7.5,7.8),
                ('CEID_NE4168','Κρυπτογραφία','Τομέας ΕΘ',5,8,1,1,'K1:A,K2:B,K4:B,K6:B',1,8.0,7.8),
                ('CEID_24EE594','Αλγοριθμικές Τεχνικές Επιστήμης δεδομένων','Τομέας ΕΘ',5,8,1,0,'K1:A,K2:B,K3:B,K4:B,K5:B,K6:B',1,7.8,7.5),
                ('CEID_NE351','Υπολογιστική Νοημοσύνη','Τομέας ΕΘ',5,8,1,1,'K1:A,K4:B,K6:A',1,8.2,7.4),
                ('CEID_NE4128','Παράλληλοι Αλγόριθμοι','Τομέας ΕΘ',5,7,0,1,'K1:B,K2:B,K6:B',1,7.7,7.8),
                ('CEID_NE5127','Αλγόριθμοι Επικοινωνιών','Τομέας ΕΘ',5,7,0,1,'K1:B,K2:B',1,7.4,7.1),
                ('CEID_NE520','Αλγόριθμοι και Εφαρμογές Τεχνητής Νοημοσύνης για το Διαδίκτυο (ΙοΤ)','Τομέας ΕΘ',5,8,0,1,'K1:B,K2:A,K3:B,K4:B,K5:B,K6:B',1,7.9,7.5),
                ('CEID_NE509','Αλγοριθμική Θεωρία Παιγνίων','Τομέας ΕΘ',5,8,0,1,'K1:B',1,7.8,7.5),
                ('CEID_NE5237','Στατιστικές Μέθοδοι Μηχανικής Μάθησης','Τομέας ΕΘ',5,7,0,1,'K1:B,K3:A,K6:B',1,8.0,7.5),
                ('CEID_NE5078','Τεχνολογίες Υλοποίησης Αλγορίθμων','Τομέας ΕΘ',5,8,0,1,'K1:B,K4:B,K6:B',1,7.5,7.8),
                ('CEID_NE5288','Ειδικά Θέματα Υπολογιστικής Λογικής','Τομέας ΕΘ',5,8,0,1,'K1:B',1,7.3,7.5),
                ('CEID_NE4017','Mαθηματική Λογική και Εφαρμογές της','Τομέας ΕΘ',5,8,0,1,'K1:B',1,7.1,7.0),
                ('CEID_NE5038','Σημασιολογία στην Επιστήμη των Υπολογιστών','Τομέας ΕΘ',5,7,0,1,'K1:B',1,7.2,7.0),
                ('CEID_NE4338','Πολυδιάστατες Δομές Δεδομένων','Τομέας ΛΥ',5,7,0,1,'K1:B,K5:B,K6:A',1,7.5,7.2),
                ('CEID_NE565','Ανάπτυξη βιντεοπαιγνιδιών','Τομέας ΕΘ',5,7,0,1,'K1:B,K2:B,K5:B',1,8.5,7.0),
                ('CEID_NE548','Εισαγωγή στη Βιοπληροφορική','Τομέας ΛΥ',5,8,0,1,'K1:B,K3:B,K5:B,K6:B',1,7.6,7.2),
                ('CEID_NE590','Κυβερνοασφάλεια','Τομέας YA',5,7,0,1,'K1:B,K2:A,K3:A,K4:B,K6:B',1,8.9,8.0),
                ('CEID_25EE606','Τοπολογική Ανάλυση Δεδομένων','Τομέας ΕΘ',5,8,0,1,'K1:B,K6:B',1,7.5,7.5),
                ('CEID_NEM04','Ρομποτική','Συνδιδασκαλία',5,8,0,1,'K1:B,K3:A,K4:B',1,7.5,7.3),
                ('CEID_24EE596','Μέθοδοι Μητρώων και Υπολογιστικά Εργαλεία','Τομέας ΛΥ',5,8,0,1,'K1:B,K3:B,K6:B',1,7.3,7.5),
                
                -- Κ2 Κατεύθυνση
                ('CEID_NE592','Βασικές Αρχές Δικτύων Κινητών','Τομέας YA',5,7,1,0,'K2:A',1,7.5,7.0),
                ('CEID_NE489','Ευφυείς Τεχνολογίες Ασύρματων Επικοινωνιών','Τομέας YA',5,7,1,0,'K2:A,K4:B',1,7.3,7.0),
                ('CEID_NE577','Αρχιτεκτονικές δικτύων επόμενης γενιάς','Τομέας YA',5,8,1,0,'K2:A',1,7.4,7.2),
                ('CEID_NE575','Υλοποιήσεις και Εφαρμογές Ασφάλειας Δικτύων','Τομέας YA',5,7,1,0,'K2:A',1,7.8,7.5),
                ('CEID_NE5168','Ευρυζωνικές Τεχνολογίες','Τομέας ΕΘ',5,8,1,0,'K2:A',1,7.2,7.0),
                ('CEID_NE574','Οπτικά Δίκτυα Επικοινωνιών','Τομέας YA',5,7,0,1,'K2:B',1,7.2,7.0),
                ('CEID_NE4157','Δίκτυα Δημόσιας Χρήσης','Τομέας ΕΘ',5,7,0,1,'K2:B',1,7.4,7.0),
                ('CEID_NE5178','Τηλεματική και Νέες Υπηρεσίες','Τομέας ΕΘ',5,8,0,1,'K2:B',1,7.2,6.8),
                ('CEID_NE4547','Τεχνικές εκτίμησης υπολογιστικών συστημάτων','Τομέας ΛΥ',5,7,0,1,'K2:B,K5:B,K6:B',1,7.0,6.8),
                ('CEID_NE576','Διάχυτος Υπολογισμός','Τομέας ΛΥ',5,8,0,1,'K2:B,K5:A,K6:B',1,7.4,7.0),
                
                -- Κ3 Κατεύθυνση
                ('CEID_NE4828','Επεξεργασία και Ανάλυση Εικόνας','Τομέας YA',5,8,1,0,'K3:A,K4:B',1,7.8,7.5),
                ('CEID_NE5597','Ανάκτηση Πληροφορίας','Τομέας ΛΥ',5,7,1,0,'K3:A,K5:B,K6:A',1,7.8,7.2),
                ('CEID_NE4847','Στατιστική Επεξεργασία Σήματος','Τομέας YA',5,7,1,0,'K3:A',1,7.6,7.3),
                ('CEID_25EE609','Επεξεργασία και Ανάλυση Video','Τομέας YA',5,8,0,1,'K3:B,K6:A',1,8.0,7.5),
                ('CEID_NE594','Αρθρωτά Κβαντικά Συστήματα','Τομέας YA',5,7,0,1,'K3:B,K4:B',1,7.0,7.0),
                ('CEID_25EE608','Αρχές Ψηφιακού Ελέγχου','Τομέας YA',5,8,0,1,'K3:B',1,7.1,6.8),
                ('CEID_NE5657','Επεξεργασία φυσικής γλώσσας','Τομέας ΛΥ',5,7,0,1,'K3:B',1,7.6,7.4),
                ('CEID_N2M05','Διαδραστικές Τεχνολογίες','Τμήμα Ηλεκτρολόγων',5,7,0,1,'K3:B',1,8.3,7.2),
                ('CEID_NE579','Εφαρμογές Ψηφιακής Επεξεργασίας Σημάτων','Τομέας YA',5,8,0,1,'K3:B,K4:B',1,7.4,7.2),
                ('CEID_NE588','Ενσωματωμένα Συστήματα','Τομέας YA',5,8,0,1,'K3:B,K4:A',1,7.6,7.2),
                ('CEID_NE593','Επεξεργασία Σημάτων και Μάθηση','Τομέας YA',5,8,0,1,'K3:B',1,7.5,7.3),
                
                -- Κ4 Κατεύθυνση
                ('CEID_NE5678','Σχεδιασμός Συστημάτων Ειδικού Σκόπου','Τομέας YA',5,7,1,0,'K4:A',1,7.0,7.0),
                ('CEID_NE4648','Εισαγωγή σε VLSI','Τομέας YA',5,7,1,0,'K4:A',1,7.2,7.5),
                ('CEID_NE4617','Προχωρημένα Θέματα Αρχιτεκτονικής','Τομέας YA',5,8,1,0,'K4:A',1,7.4,7.5),
                ('CEID_NE4658','Σχεδίαση Συστημάτων με Χρήση CAD','Τομέας YA',5,8,1,0,'K4:A',1,7.1,7.2),
                ('CEID_NE591','Ασφάλεια σε Υλικό','Τομέας YA',5,8,1,0,'K4:A',1,7.5,7.5),
                ('CEID_NE5647','Σχεδιασμός Συστημάτων VLSI','Τομέας YA',5,8,1,0,'K4:A',1,7.2,7.1),
                ('CEID_NE5668','Ειδικά Θέματα Ψηφιακών Συστημάτων','Τομέας YA',5,8,0,1,'K4:B',1,7.3,7.0),
                
                -- Κ5 Κατεύθυνση
                ('CEID_NE5407','Λογισμικό Συστημάτων Υψηλής Επίδοσης','Τομέας ΛΥ',5,7,1,0,'K5:A,K6:A',1,7.8,8.0),
                ('CEID_24EE595','Διαχείριση έργων λογισμικού με Agile','Τομέας ΛΥ',5,8,1,0,'K5:A',1,8.3,6.0),
                ('CEID_NE5577','Ποιότητα Λογισμικού','Τομέας ΛΥ',5,7,1,0,'K5:A',1,7.5,6.5),
                ('CEID_NE584','e-Επιχειρείν','Τομέας ΛΥ',5,7,0,1,'K5:B',1,7.5,5.8),
                ('CEID_NE5908','Κοινωνικές και Νομικές Πλευρές','Τομέας ΛΥ',5,8,0,1,'K5:B',1,7.2,5.5),
                ('CEID_NE444','Αναπαράσταση Γνώσης στον Ιστό','Τομέας ΛΥ',5,7,0,1,'K5:B',1,7.4,7.1),
                ('CEID_NE1411','Τεχνολογίες Αποκεντρωμένων Δεδομένων','Τομέας ΛΥ',5,7,0,1,'K5:B,K6:B',1,7.8,7.5),
                
                -- Κ6 Κατεύθυνση
                ('CEID_NE4348','Συστήματα Μεγάλων Δεδομένων','Τομέας ΛΥ',5,8,1,0,'K6:A,K5:B,K3:B',1,8.2,7.8),
                ('CEID_NE562','Εξόρυξη Δεδομένων και Μηχανική Μάθηση','Τομέας ΛΥ',5,8,1,0,'K6:A,K5:B,K3:B',1,8.6,7.5),
                ('CEID_25EE607','Παράλληλη Επεξεργασία','Τομέας ΛΥ',5,8,1,0,'K6:A,K5:A',1,7.9,8.0),
                ('CEID_25EX601','Επιστημονικός Υπολογισμός','Τομέας ΛΥ',5,7,0,1,'K6:B',1,7.3,7.1),
                ('CEID_NE5358','Εισαγωγή στα Πληροφοριακά Συστήματα','Τομέας ΛΥ',5,8,0,1,'K6:B',1,7.4,6.5),
                
                -- ΜΑΘΗΜΑΤΑ ΓΕΝΙΚΗΣ ΠΑΙΔΕΙΑΣ (ΓΠ)
                ('CEID_E90E','Εισαγωγή στα Οικονομικά','Μάθημα Γενικής Παιδείας',5,7,0,0,'EXTERNAL',1,8.0,5.0),
                ('CEID_ΝΠΑ','Πρακτική Άσκηση','Μάθημα Γενικής Παιδείας',5,8,0,0,'EXTERNAL',1,9.0,4.0),
                ('CEID_ΓΠ37','Φιλοσοφία του νου','Μάθημα Γενικής Παιδείας',3,7,0,0,'EXTERNAL',1,7.5,4.5),
                ('CEID_ΓΠ7Ο','Η Βυζαντινή Φιλολογία στον 21ο αιώνα','Μάθημα Γενικής Παιδείας',3,7,0,0,'EXTERNAL',1,7.2,4.0),
                ('CEID_AE2','Εισαγωγή στο Μάρκετινγκ','Μάθημα Γενικής Παιδείας',3,8,0,0,'EXTERNAL',1,8.2,5.0),
                ('CEID_ΔΕΖ','Δημόσια Οικονομική','Μάθημα Γενικής Παιδείας',3,8,0,0,'EXTERNAL',1,7.4,5.2),
                ('CEID_ERA1','Μάθημα Erasmus - Χειμερινό','Πρόγραμμα Ανταλλαγής',5,7,0,0,'EXTERNAL',1,8.5,5.0),
                ('CEID_ERA2','Μάθημα Erasmus - Εαρινό','Πρόγραμμα Ανταλλαγής',5,8,0,0,'EXTERNAL',1,8.5,5.0)
            """);
            System.out.println("✅ Courses (100% Ευθυγραμμισμένα με το PDF) προστέθηκαν!");
            ...
            conn.createStatement().execute("""
                ...
                VALUES 
                (1,'Πρώτο Σενάριο', 'Μία κύρια Κατεύθυνση: 5A+5B+5A από 3 άλλες+2 ελεύθερα', 60, 30),
                (2,'Δεύτερο Σενάριο', 'Δύο κύριες Κατευθύνσεις: (5A+2B)+(5A+2B)+3 ελεύθερα', 45, 45),
                (3,'Τρίτο Σενάριο', 'Γενικής Κατεύθυνσης: 10 Α από όλες+7 ελεύθερα', 30, 30)
            """);
            ...
            conn.createStatement().execute("""
                ...
                (planId,studentId,scenarioId,academicYear,directionName,mainDirection,secondDirection,totalECTS,status,isValid,isDraft,isFinalized,workLoadIndex,createdDate,lastUpdatedDate)
                VALUES 
            ...
            """);
            System.out.println("Base Status: ✅ StudyPlans (25) προστέθηκαν!");

            // ── THESIS ───────────────────────────────────────────────
            conn.createStatement().execute("""
                INSERT OR IGNORE INTO Thesis 
                (thesisId,professorId,title,description,prerequisites,requiredECTS,isAvailable,maxCandidates,interestedStudents,requiredSkills,publicationDate,lastModifiedDate)
                VALUES 
                (1,1,'Παράλληλοι Αλγόριθμοι σε Κατανεμημένα Συστήματα','Υλοποίηση παράλληλων αλγορίθμων βελτιστοποίησης','Αλγόριθμοι, Κατανεμημένα Συστήματα',180,1,2,1,'Python,MPI',datetime('now'),datetime('now')),
                (2,2,'Σύστημα Διαχείρισης Μεγάλων Δεδομένων','Σχεδίαση Big Data συστήματος','Βάσεις Δεδομένων, Κατανεμημένα',160,1,3,2,'MongoDB,Spark',datetime('now'),datetime('now')),
                (3,3,'Αλγόριθμοι ML για Ανάλυση Γράφων','Machine learning σε δίκτυα','ΤΝ, Θεωρία Γραφημάτων',200,1,2,0,'Python,PyTorch',datetime('now'),datetime('now')),
                (4,4,'Πλατφόρμα Διαχείρισης Ακαδημαϊκής Πορείας','Εφαρμογή για φοιτητές CEID','Τεχνολογία Λογισμικού, Βάσεις',150,1,4,3,'Java,JavaFX,SQLite',datetime('now'),datetime('now')),
                (5,1,'Κρυπτογραφικά Πρωτόκολλα για IoT','Ελαφριά κρυπτογραφία για IoT','Κυβερνοασφάλεια, Δίκτυα',180,1,2,1,'C/C++,Cryptography',datetime('now'),datetime('now')),
                (6,5,'Σύστημα Εξόρυξης Δεδομένων από Ιστοσελίδες','Web scraping και NLP','Προγραμματισμός Ιστού, ΤΝ',150,1,3,2,'Python,NLTK',datetime('now'),datetime('now')),
                (7,2,'Βελτιστοποίηση Ερωτημάτων σε Κατανεμημένες Βάσεις','Query optimization','Βάσεις, Κατανεμημένα',170,0,2,2,'SQL,Spark',datetime('now'),datetime('now')),
                (8,7,'Ασφάλεια Δικτύων Επόμενης Γενιάς','Security σε 5G networks','Δίκτυα, Κυβερνοασφάλεια',180,1,2,1,'Python,Networking',datetime('now'),datetime('now')),
                (9,11,'Βαθιά Μάθηση για Αναγνώριση Εικόνας','Deep learning για image recognition','ΤΝ, Επεξεργασία Εικόνας',190,1,3,2,'Python,TensorFlow',datetime('now'),datetime('now')),
                (10,13,'Αλγόριθμοι για Δίκτυα Αισθητήρων IoT','Wireless sensor networks','Κατανεμημένα, Αλγόριθμοι',175,1,2,0,'C,Python,ns-3',datetime('now'),datetime('now'))
            """);
            ...
            conn.createStatement().execute("""
                ...
                (20,12,'CEID_NE4828',datetime('now'),1,8,'Computer vision rocks',1)
            """);
            ...
            // ── NOTIFICATIONS ────────────────────────────────────────
            // 🌟 ΔΙΟΡΘΩΣΗ: Τα recipientId και senderId ταιριάζουν πλέον 100% με τα userId του πίνακα User
            conn.createStatement().execute("""
            ...
            conn.createStatement().execute("""
                ...
                (4,'CEID_24Y332','teaches'),(4,'CEID_24Y338','manages'),
